...
export const HomeAssistantSettingsPanel: React.FC<HomeAssistantSettingsPanelProps> = ({ enabled, onToggleEnabled }) => {
  ...
  const statusDot = connectionError
    ? 'bg-red-400'
    : isConfigured
      ? 'bg-emerald-400'
      : 'bg-amber-400/60';
  ...
  return (
    <details className="@container/ha rounded-2xl border border-outline-variant/20 bg-surface-container overflow-hidden">
      <summary className="flex cursor-pointer select-none list-none items-center justify-between gap-3 bg-surface-container-low px-5 py-4">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-[#03A9F4]/10 rounded-xl shrink-0">
            <Server className="w-4 h-4 text-[#03A9F4]" />
          </div>
          <div>
            <p className="font-bold text-sm text-on-surface">Home Assistant</p>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div
            ...
            tabIndex={0}
            onClick={e => { e.stopPropagation(); toggleEnabled(!enabled); }}
            onKeyDown={event => {
